import { Base64 } from 'js-base64';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { ServiceAccount } from './serviceAccount';

const cryptoMock = {
  subtle: {
    importKey: vi.fn(),
  },
};

vi.stubGlobal('crypto', cryptoMock);

describe('ServiceAccount', () => {
  let serviceAccount: ServiceAccount;

  beforeEach(() => {
    serviceAccount = new ServiceAccount(
      'project_id',
      'aaaaaaaaaabbbbbbbbbbccccccccccdddddddddd',
      '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
      'foo@project_id.iam.gserviceaccount.com',
    );
  });

  afterEach(() => {
    vi.clearAllMocks();
  });

  it('Should import private key', async () => {
    const withoutLineBreaks =
      '';

    await serviceAccount.getPrivateKey();
    const buffer = Base64.toUint8Array(withoutLineBreaks);

    const algorithm = {
      name: 'RSASSA-PKCS1-v1_5',
      hash: {
        name: 'SHA-256',
      },
    };

    const keyUsages = ['sign'];

    expect(crypto.subtle.importKey).toBeCalledTimes(1);
    expect(crypto.subtle.importKey).toBeCalledWith(
      'pkcs8',
      buffer,
      algorithm,
      false,
      keyUsages,
    );
  });
});
